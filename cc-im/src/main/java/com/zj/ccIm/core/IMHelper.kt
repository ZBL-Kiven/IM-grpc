...
object IMHelper {
    ...
    fun leaveChatRoom(key: String) {
        ...
        if (r != null) {
            CcIM.send(r, Constance.CALL_ID_LEAVE_CHAT_ROOM + r.key, Constance.SEND_MSG_DEFAULT_TIMEOUT, isSpecialData = false, ignoreConnecting = false, ignoreSendState = true)
        }
    }
    ...
    internal fun resumedChatRoomIfConnection(req: ChannelRegisterInfo): String? {
        ...
        CcIM.send(req, Constance.CALL_ID_REGISTER_CHAT + req.key, Constance.SEND_MSG_DEFAULT_TIMEOUT, isSpecialData = false, ignoreConnecting = false, ignoreSendState = true)
        return req.key
    }
