...
open class MsgSender internal constructor(private val config: SendMsgConfig) {
    ...
    private fun send(sen: SendMessageReqEn, isRecent: Boolean = false, sendBefore: OnSendBefore<Any?>? = FileSender.getIfSupport(sen)) {
        ...
        if (sendBefore == null || p?.first == true) {
            val sb = if(sendBefore == null) arrayOf() else arrayOf(sendBefore)
            IMChannelManager.sendMsgWithChannel(sen, sen.clientMsgId, Constance.SEND_MSG_DEFAULT_TIMEOUT, isSpecialData = sen.ignoreSendConditionState, ignoreConnecting = sen.ignoreConnectionState, ignoreSendState = sen.sendWithoutState, isRecent,sendBefore = sb ,config.customSendListener?.callback)
        } else {
