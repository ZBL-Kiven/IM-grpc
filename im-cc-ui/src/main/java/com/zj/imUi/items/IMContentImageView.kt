...
class IMContentImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, def: Int = 0) : AppCompatImageView(context, attrs, def), ImContentIn {
    ...
    private fun loadImg(data: ImMsgIn, arrayInt: Array<Int>) {
        ...
        layoutParams = lp
        val imgUrl: String? = if (data.getReplyMsgType() == UiMsgType.MSG_TYPE_IMG && data.getReplyMsgImgContent() != null) {
    ...
    private fun setImgLp(data: ImMsgIn): Array<Int> {
        ...
        val imgWidth = if (data.getImgContentWidth() != null && data.getImgContentHeight() != null) {
