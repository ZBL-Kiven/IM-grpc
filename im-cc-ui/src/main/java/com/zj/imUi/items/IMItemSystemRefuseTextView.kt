...
class IMItemSystemRefuseTextView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, def: Int = 0) : BaseBubble(context, attrs, def) {
    ...
    override fun init(data: ImMsgIn, chatType: Any) {
        ...
        if (data.getSelfUserId() == data.getSenderId()) {
            tvContent.text = "我拒绝了一条消息"
        } else tvContent.text = context.getString(R.string.im_chat_msg_owner_refuse)
    }
