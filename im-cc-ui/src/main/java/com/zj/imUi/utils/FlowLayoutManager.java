...
public class FlowLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            ...
            if ((i+1)%5!=0&&!isLineChange)
                isAdd = true;
            else if( (i + 1) % 5 == 0 && !isLineChange)
                isAdd = false;
            else if (isLineChange)
                isAdd = true;
            // 如果加上当前的item还小于最大的宽度的话,或者item数量等于4的倍数时，换行
            if (cuLineWidth + childUseWidth <= usedMaxWidth && isAdd) {
    ...
    private void fillLayout(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        Rect displayFrame =
                new Rect(
                        getPaddingLeft(),
                        getPaddingTop() + verticalScrollOffset,
    ...
    private void formatAboveRow() {
        ...
        for (int i = 0; i < views.size(); i++) {
            ...
            if (allItemFrames.get(position).top
                    < row.cuTop + (row.maxHeight - views.get(i).useHeight) / 2) {
                ...
                frame.set(
                        ...
                        (int) (row.cuTop + (row.maxHeight - views.get(i).useHeight) / 2),
                        allItemFrames.get(position).right,
                        (int)
                                (row.cuTop
                                        + (row.maxHeight - views.get(i).useHeight) / 2
    ...
    public int scrollVerticallyBy(
        ...
        if (verticalScrollOffset + dy < 0) { // 限制滑动到顶部之后，不让继续向上滑动了
            ...
            travel =
                    totalHeight
                            - getVerticalSpace()
                            - verticalScrollOffset; // verticalScrollOffset=totalHeight -
            // getVerticalSpace()
        }
